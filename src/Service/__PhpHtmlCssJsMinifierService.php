...
class __PhpHtmlCssJsMinifierService
{
    /**
     * ----------------------------------------------------------------------------------------
    ...
    protected function __minify_x($input) {
        return str_replace(array("\n", "\t", ' '), array(X . '\n', X . '\t', X . '\s'), $input);
    }

    protected function __minify_v($input) {
            ...
            if(isset($m[2])) {
                ...
                if(stripos($m[2], ' style=') !== false) {
                    $m[2] = preg_replace_callback('#( style=)([\'"]?)(.*?)\2#i', function($m) {
                        return $m[1] . $m[2] . $this->getMinifiedCss($m[3]) . $m[2];
                    }, $m[2]);
                }
    ...
    public function getMinifiedHtml($input) {
    ...
    public function getMinifiedCss($input) {
        if( ! $input = trim($input)) return $input;
        global $SS, $CC;
    ...
    public function getMinifiedJs($input) {
        if( ! $input = trim($input)) return $input;
